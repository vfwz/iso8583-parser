...
public class Iso8583Message {
    ...
    private final Map<Integer, Iso8583Field> values = new TreeMap<Integer, Iso8583Field>();
    private String tpdu = "";
    ...
    public Iso8583Message setValue(int index, String value) {
        ...
        if (factory.isBit128() && index > 128) {
            return this;
        }
        if (!factory.isBit128() && index > 64) {
            return this;
        }
    ...
    public String toFormatString() {
        StringBuilder sb = new StringBuilder();
        String formatHead = "%s:[%s]";
        String format = "F%s:[%s]";
        sb.append("\n");
        sb.append(String.format(formatHead, Iso8583ConstantValue.TPDU.getValue(), getTpdu()));
        sb.append("\n");
        sb.append(String.format(formatHead, Iso8583ConstantValue.HEADER.getValue(), getHeader()));
        sb.append("\n");
        sb.append(String.format(formatHead, Iso8583ConstantValue.MTI.getValue(), getMti()));
        sb.append("\n");
        sb.append(String.format(formatHead, Iso8583ConstantValue.BITMAP.getValue(), getBitmapString()));
        sb.append("\n");
        for (Map.Entry<Integer, Iso8583Field> entry : values.entrySet()) {
            Iso8583Field field = entry.getValue();
            String index = field.getIndex() < 10 ? "0" + field.getIndex() : Integer.toString(field.getIndex());
    ...
    public byte[] getBytes() {
        ...
        try {
            ...
            byte[] bSize = EncodeUtil.hex2Bytes(size);
            //准备最终报文数据
    ...
    private byte[] parse(String value, Iso8583FieldType type) {
        ...
        if (null != value) {
            result = type.encodeValue(value, this.factory.getCharset());
        }
        return result;
    }
    ...
    private byte[] numericVariableLengthParse(String value, int dataLength, boolean isByteLength, Iso8583FieldType type) {
        Iso8583FillBlankStrategy strategy = type.getFillBlankStrategy();
        boolean lengthWithBlank = strategy != null && strategy.isLengthWithBlank();
        int length = getFieldValue(value, type, lengthWithBlank).length();
    ...
    private String getFieldValue(String value, Iso8583FieldType type, boolean filledValue) {
        if (Iso8583FieldType.FieldTypeValue.LLVAR_NUMERIC == type.getFieldTypeValue()
                ...
                || Iso8583FieldType.FieldTypeValue.LLVAR_HEX == type.getFieldTypeValue()
                || Iso8583FieldType.FieldTypeValue.LLLVAR_HEX == type.getFieldTypeValue()
                || Iso8583FieldType.FieldTypeValue.LLLLVAR_HEX == type.getFieldTypeValue()
        ) {
            //获取补齐结果与非补齐结果报文内容
            ...
            if (0 == value.length() % 2) {
                ...
                if (fillBlankStrategy == null) {
                    log.error("域[{}]类型为[{}], 当前值[{}]长度为奇数，需要设置填充方案",
                            type.getFieldIndex(), type.getFieldTypeValue().toString(), value);
                    throw new Iso8583Exception("域[" + type.getFieldIndex() + "]类型为[" + type.getFieldTypeValue().toString() + "], 当前值[" + value + "]长度为奇数，需要设置填充方案");
                }
                //用于填充的填充字符
    ...
    private byte[] variableLengthParse(String value, String fieldLength, int dataLength, boolean isByteLength, boolean isNumeric) {
        //计算bcd长度位
        byte[] bLength = EncodeUtil.hex2Bytes(fieldLength.substring(fieldLength.length() - dataLength * 2));
        //获取报文的字节数组
        byte[] bContent = isByteLength ? (isNumeric ? EncodeUtil.hex2Bytes(value) : value.getBytes(this.factory.getCharset())) : EncodeUtil.hex2Bytes(value);
        //数据拼接 length+data
